...
function setup() {
  ...
  socket.on('search', function(results){
    console.log("sending "+ results.final + ", " + results.id);
  });
...
function updateSearch(){
  ...
  var final = [keyword, s_year, s_month, e_year, e_month, choice_news]
...
function drawText(data) {
  ...
  for (var i = 1; i < 6; i++){
    if(i == 1){
    b1 = createButton(words[i-1]);
    b1.position(10, 500 + i*30);
    }
    if(i == 2){
    b2 = createButton(words[i-1]);
    b2.position(10, 500 + i*30);
    }
    if(i == 3){
    b3 = createButton(words[i-1]);
    b3.position(10, 500 + i*30);
    }
    if(i == 4){
    b4 = createButton(words[i-1]);
    b4.position(10, 500 + i*30);
    }
    if(i == 5){
    b4 = createButton(words[i-1]);
    b4.position(10, 500 + i*30);
 ...
 console.log(b1);
 if(b1.mouseClicked){
    console.log('button of article')
    articleId(1);
 }
 else{console.log("nothing clicked");}
}
