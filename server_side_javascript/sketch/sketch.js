...
function setup() {
  background(236, 215, 255);
  image(logoImg, windowWidth/12, 200);
...
function drawVis(){
  console.log("drawVis called");
  img1.resize(400,400);
  img2.resize(400, 300);
  image(img1, windowWidth/2 - 150, windowHeight/2+ 20);
  image(img2, windowWidth/2 + img1.width - 150, windowHeight/2 + 20);
}

function draw(){
  summHeader.position(windowWidth/2-150, 30);
  collHeader.position(10, windowHeight/2 - 50);
  visHeader.position(windowWidth/2-150, windowHeight/2 - 50);
...
function keyTyped(){
    if(key === '1'){
      ...
      console.log("start running vis code");
      id = 9;
      sendResults(final, id);
    }
    else if(key === 'i'){
      console.log("loading vis results");
      loadVis();
    }
    else if(key === 'o'){
      console.log("drawing vis results");
      drawVis();
    }
    else if(key === 'p'){
      console.log("s typed. summary will be loaded");
      loadStrings("summary.txt", summLoaded);
    }
    else if(key === 'u'){
      loadJSON("search.json", gotData, 'json');
    }
...
function summLoaded(data){
  ...
  console.log(data)
  textSize(15);
  text(data, windowWidth/2 -150, 100, 500, 300);
}

function windowResized() {
  console.log("window Resized, drawAgain");
